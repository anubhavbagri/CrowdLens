...
public class SearchJobListener {
    ...
    /**
     * On startup: bulk-fail any PENDING/IN_PROGRESS jobs left over from a prior crash or restart.
     * Runs before rqueue listener threads start, so no race with message processing.
     */
    ...
    public void failIncompleteJobsOnStartup() {
        ...
        if (count > 0) {
            log.warn("Server restart — marked {} stale job(s) FAILED. Clients should resubmit.", count);
        }
    }
